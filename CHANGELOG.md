...
- `VALIDATION_RECORD.md` and the README now state the envelope, separating two claims that were easy to conflate. Results are validated against external data only over 579–717 km, the band the bundled LightSail 2 epochs span, which is the sole place anything is compared against a reference outside this repository. The engine is *exercised* from roughly 185 km upward — atmosphere tests span 200–1500 km, the decay example runs at 400 km, the GTO example has a 185 km periapsis — but those assert direction, monotonicity and order of magnitude rather than accuracy, so passing them means self-consistent, not correct. §2's archive runs to 366 km, inside the exercised range and well outside the validated one
